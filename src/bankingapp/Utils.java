...
public class Utils {
    ...
    public static double getFDAccountBalance(int accountNumber){
        
        String sql = null;
        EntityManager BankingPUEntityManager = javax.persistence.Persistence.createEntityManagerFactory("BankingAppPU").createEntityManager();
        sql="select sum(withdrawal) from transactions inner join accounts on transactions.account_id = accounts.id where accounts.id = "+accountNumber+" and transactions.lledger_head_id = "+getLedgerHead("FD").getId();
        Query getSumQuery = BankingPUEntityManager.createNativeQuery(sql);
        Double d = (Double) ((Vector) getSumQuery.getSingleResult()).get(0);
        sql="select sum(deposit) from transactions inner join accounts on transactions.account_id = accounts.id where accounts.id = "+accountNumber+" and transactions.lledger_head_id = "+getLedgerHead("FD").getId();
        getSumQuery = BankingPUEntityManager.createNativeQuery(sql);
        Double dd = (Double) ((Vector) getSumQuery.getSingleResult()).get(0);
        d= d==null ? 0 : d;
        dd= dd==null ? 0 : dd;
